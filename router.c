...
int main(int argc, char* arv[]) {
  ...
  if (argc != 6 && argc != 8 && argc != 9) {
    fprintf(stderr, "Usage: %s L B source_port_1 destination_port_1 [source_port_2 destination_port_2 duty_cycle [prioritized]]", argv[0]);
    return -1;
  }
  if (argc > 6) {
    streams = 2;
  ...
  int outsock_1 = send_port("localhost", argv[4], &p1);
  ...
  while (count < MAX_PACKETS) {
    ...
    read_count = recvfrom(insock_1, &p, sizeof(p), &src_addr, &src_len);
		...
		diff_timeval(&diff_time, &curr_time, &last_time);
      
		if (diff_time->tv_sec * 1000000 + diff_time_tv_usec > L * 1000) {
