...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    int i;
    for(i=((last_received+1)%SEQ_MAX); i != ((last_received+1+window_size)%SEQ_MAX); i = (i+1)%SEQ_MAX){
      ...
      if(diff_time.tv_sec * 1000000 + diff_time.tv_usec > rtt){
        ...
        available_window = (available_window+1) / 2;
        break;
      } 
    }
    ...
    if(retransmitting != -1){
      ...
      do {
        ...
        total_attempts++;
        fuckups++;
      } while (i != (retransmitting+window_size)%SEQ_MAX);
    }
    ...
    if(bytes_read > 0){
      ...
      if(rcv_pkt.stream == 'Z' && rcv_pkt.seq_number == (last_received + 1) % (SEQ_MAX)){
        ...
        available_window += 1;
        last_received = rcv_pkt.seq_number % (SEQ_MAX);
        acks++;
        if (acks > window_size) {
          window_size++;
          available_window++;
