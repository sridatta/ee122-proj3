...
int main(int argc, char *argv[]){
  ...
  bytequeue q;
  bytequeue_init(&q, window_size*sizeof(ee122_packet), window_size);
  ...
  while(1){
    ...
    timeval_subtract(&diff_time, &start_time, &curr_time);
    ...
    for(i=0; i < window_size; i++){
      ...
      timeval_subtract(&diff_time, &timeouts[i], &curr_time);
      if(diff_time.tv_sec * 1000000 + diff_time.tv_usec > rtt*1000000){
        retransmitting = 1;
      }
    ...
    timeval_subtract(&diff_time, &last_generated, &curr_time);
    ...
    if(bytes_read > 0){
      ...
      if(pkt.stream = 'Z'){
        struct timeval timeout_start = timeouts[pkt.seq_number];

        // Learn RTT
        timeval_subtract(&diff_time, &timeout_start, &curr_time);
